# Agent setup for `deepsec`

This is a deepsec scanning workspace. Project `deepsec` was just registered
(target: `..`). Setup is incomplete — `data/deepsec/INFO.md`
...
```bash
pnpm deepsec scan    --project-id deepsec
pnpm deepsec process --project-id deepsec
```
